...
describe('InviteTokenPanel (model.md §5 canInvite gating)', () => {
  it('renders the invite token for the owner', async () => {
    await renderWithQueryClient(<InviteTokenPanel pool={makePool()} inviteToken="AB3XK9M2" viewerIsOwner={true} />);
  ...
  it('calls onCopy with the token and shows "Copied!" feedback when the copy button is pressed', async () => {
    ...
    await renderWithQueryClient(<InviteTokenPanel pool={makePool()} inviteToken="AB3XK9M2" viewerIsOwner={true} onCopy={onCopy} />);
