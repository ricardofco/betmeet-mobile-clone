...
describe('PoolOverridePicker (PREDICTIONS-3, design.md §4/§9)', () => {
  it('renders nothing when the viewer belongs to no pools', async () => {
    const { toJSON } = await render(<PoolOverridePicker pools={[]} selectedPoolId={null} onSelect={jest.fn()} />);
    expect(toJSON()).toBeNull();
  });

  it('renders a Global chip plus one chip per pool', async () => {
    await render(<PoolOverridePicker pools={POOLS} selectedPoolId={null} onSelect={jest.fn()} />);
  ...
  it('calls onSelect(null) when the Global chip is pressed', async () => {
    ...
    await render(<PoolOverridePicker pools={POOLS} selectedPoolId="p1" onSelect={onSelect} />);
  ...
  it('calls onSelect(poolId) when a pool chip is pressed', async () => {
    const onSelect = jest.fn();
    const user = userEvent.setup();
    await render(<PoolOverridePicker pools={POOLS} selectedPoolId={null} onSelect={onSelect} />);
