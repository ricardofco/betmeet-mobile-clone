...
|---|---|---|
| **Backend API** (standalone `backend/` Node/Express service **inside this repo**, own Supabase project — resolved 2026-07-01, ADR-026, supersedes "hosting TBD" below) | All business-rule-bearing mutations and reads from `domain-overview.md §5`: nickname/discriminator assignment, pool capacity/invite/membership rules, prediction validation + save, the authoritative scoring run, notification preference/dispatch, admin overrides, sync-dashboard reads | REST/RPC over HTTPS, bearer-token authenticated (the user's Supabase session JWT), versioned contract (see §3) |
