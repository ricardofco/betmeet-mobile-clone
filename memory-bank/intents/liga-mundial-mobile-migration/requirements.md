...
- **RESOLVED 2026-07-01 (ADR-026, `memory-bank/bolts/bolt-backend-phase1/`):** the hosting question above is now settled — the backend lives inside **this repo**, as a standalone `backend/` Node/Express service against the user's own, separate Supabase project. `betmeet-clone` is not modified and stays web-only independent. See ADR-026 through ADR-029 for the full decision, the Access Token Hook porting requirement (ADR-027), and the phased build order (ADR-028).
