...
## Overall Status
- **Architecture pivot in progress (2026-07-01):** backend hosting resolved (ADR-026) — a standalone `backend/` Node/Express service now being built **inside this repo**, own Supabase project, `betmeet-clone` untouched/independent. Phased build (ADR-028): Phase 1 (in progress) unblocks Layer 2 testing for Bolts 1/2/3/5/6 by implementing exactly the capabilities mobile already calls, plus a ported Auth Hook migration (ADR-027, required for Bolt 1's claims-based guard to work on the new Supabase project) and manually-seeded competition data. Phases 2-4 (admin, sync+scoring+cron, FCM/APNs dispatch) follow separately. See `memory-bank/bolts/bolt-backend-phase1/`.
- **Current Phase:** Construction — **Bolt 6 (Predictions core) closed at Layer 1.** This is the first bolt to mount a real, navigable Predictions screen — Layer 2 not run in this environment (no simulator available); 10 manual test paths documented, including Bolt 5's six previously-deferred paths, now re-scoped, and now actually runnable once backend-phase1 Phase 1 is live. **Bolt 7 (Pools core) is next per bolt-plan sequencing — its only dependency (Bolt 3) is already done** (unaffected by the backend pivot; pools capabilities are explicitly out of backend Phase 1).
- **Bolts Completed:** 6.5 / 13 (Bolt 0 ✅, Bolt 1 ✅ both layers; Bolt 2 Layer 1 ✅ / Layer 2 pending manual; Bolt 3 Layer 1 ✅ / Layer 2 pending manual; Bolt 4 Layer 1 ✅ / Layer 2 pending manual; Bolt 5 Layer 1 ✅ / Layer 2 deferred to Bolt 6 (now addressed); Bolt 6 Layer 1 ✅ / Layer 2 pending manual)
...
| 7 | Pools core | Bolt 3 | Low-Medium | Not started — **dependency (3) already done** |
| 8 | Pools advanced + predictions↔pools integration + account deletion | Bolt 6, 7 | Medium-High | Not started — **dependency Bolt 6 now done, still waiting on Bolt 7** |
